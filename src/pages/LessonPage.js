...
class component extends Component {
	...
	showTab = (tab, e) => {
		if(tab === 1) {
			...
			this.resizeView(0);
		else if(this.option.tab_width < 300)
			this.resizeView(300)
	...
	renderButtons() {
		return (
			<div className="btn-wrapper">
				<a className="btn btn-default zoom-in-btn" role="button" onClick={this.addZoom}>+</a>
				<a className="btn btn-default zoom-out-btn" role="button" onClick={this.minusZoom}>-</a>
			</div>
		)
	}
	...
	renderEdit() {
		...
		return (<div className="lesson-page-edit-area" style={{"display":is_show}}>
			...
			<div className="lesson-page-edit-btn-area">
				<button className="lesson-page-edit-save" onClick={this.saveEdit}>저장</button>
				<button className="lesson-page-edit-cancel" onClick={this.cancelEdit}>취소</button>
			</div>
		</div>)
	}
	...
	render() {
		...
		const tabs = memberStatus === "INSTRUCTOR" ? [1,2,3,4] : [2,3,4];
		
	    const html = (<div className="lesson-body" onDragOver={this.dragover}  >
	    {this.renderButtons()}
	    	...
	    	<div className="tab-wrapper" ref="tabwrapper">
	    		<ul className="tabs" ref="tab-mover" draggable="true" onDragStart={this.dragstart} onDragOver={this.dragover} onDragEnd={this.dragend} >
						{tabs.map((tab, i) => (
			    		<li onClick={(e)=>{this.showTab(tab, e)}} key={i} className={classNames({"selected":this.state.tab === tab})} data-tab={tab}></li>
	    			))}
